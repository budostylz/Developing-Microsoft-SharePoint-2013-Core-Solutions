...
$privateKeyPath = $certFolder + $remoteDomain + ".ptx"

#Call makecert.exe to create the certificate
Write-Host "Creating the new certificate..."
##Create the new certificate here
& $makecert -r -pe -n "CN=$remoteDomain" -b 07/31/2016 -e 07/31/2026 -eku 1.3.6.1.5.5.7.3.1 -ss my -sr localMachine -sky exchange -sy 12 -sp "Microsoft RSA SChannel Cryptographic Provider" $publicCertPath
